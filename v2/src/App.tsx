...
type AbandonedReportRow = { label: string; phone: string; service: Service; wait: string; waitSec: number; status: string; operatorCallback: string; userCallback: string; details: DetailItem[] };
...
function exportCsv(rows: AbandonedReportRow[]) {
  const header = ['Statut', 'Client', 'Telephone', 'Famille', 'Duree attente', 'Rappel operatrice', 'Utilisateur a deja rappele'];
  const body = rows.map((r) => [r.status, r.label, r.phone, r.service, r.wait, r.operatorCallback, r.userCallback]);
...
function App() {
  ...
  const abandonedRows = useMemo((): AbandonedReportRow[] => abandonedVisible.map((q) => {
    ...
    return { label: q.client, phone: q.phone, service: q.service, wait: fmtClock(q.wait), waitSec: q.wait, status: statusForAbandon(q, op, user), operatorCallback: op ? `${op.operator} · rappel à ${frTime(op.time)} · durée ${fmt(op.duration)}` : 'Aucun rappel operatrice trouve', userCallback: user ? `Utilisateur a deja rappele · ${frTime(user.time)} · entrant decroche · pris par ${user.operator} · durée ${fmt(user.duration)}` : 'Aucun rappel entrant ulterieur detecte', details: callDetails([q]) };
