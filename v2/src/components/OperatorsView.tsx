...
function computeSentiment(raw: OperatorRawScore, team: OperatorRawScore[]) {
  ...
  const parts: ScorePart[] = [
    ...
    { key: 'paroleScore', label: 'Parole moyenne', score: lowerIsBetter(Math.abs(raw.talkAvg - teamTalk), teamTalk || raw.talkAvg || 1), weight: 0.09, formula: 'Formule : écart entre parole moyenne opératrice et médiane équipe. Plus l’écart est maîtrisé = meilleur score.', source: `Données : opératrice ${formatClock(raw.talkAvg)} / médiane équipe ${formatClock(teamTalk)}.`, interpretation: 'Valorise une durée de conversation cohérente : ni trop expéditive, ni anormalement longue.', reason: `Parole moyenne ${formatClock(raw.talkAvg)} comparée à la médiane équipe ${formatClock(teamTalk)}.` },
